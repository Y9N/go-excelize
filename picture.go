...
//我自己的方法
/*func (f *File) SetSheetDrawingXMLAndRelationships(sheet string) (newF *File,err error) {
	var (
		wsDr *xlsxWsDr
		deTwoCellAnchors []*decodeTwoCellAnchor
	)
	ws, err := f.workSheetReader(sheet)
	if err != nil {
		return nil, err
	...
	for _, anchor := range deWsDr.TwoCellAnchor {
		...
		deTwoCellAnchors = append(deTwoCellAnchors,deTwoCellAnchor)
	}
	newF = f
	newF.WsDr = wsDr
	newF.DrawingRelationships = drawingRelationships
	newF.DeWsDr          = deWsDr
	newF.DeTwoCellAnchors = deTwoCellAnchors
	return newF,err
}*/
...
//			if drawRel := f.getDrawingRelationships(drawingRelationships, anchor.Pic.BlipFill.Blip.Embed); drawRel != nil {
//				if _, ok := supportedImageTypes[filepath.Ext(drawRel.Target)]; ok {
//					ret := filepath.Base(drawRel.Target)
//					cellObj, err := CoordinatesToCellName(anchor.From.Col+1, anchor.From.Row+1)
//					if err != nil {
//						return map[string]SheetPictures{}, err
//					}
...
//	err = nil
//	//获取图片
//	for _, deTwoCellAnchor := range deTwoCellAnchors {
...
func (f *File) GetPictureMap(sheet string, chanCount int) (sheetPictureMaps map[string]SheetPictures, err error) {
	...
	// 先获取图片和存储位置的map
	drawRels := f.getDrawingRelationshipsMap(drawingRelationships)
	// 第一种方式
	deWsDr := new(decodeWsDr)
	if err = f.xmlNewDecoder(bytes.NewReader(namespaceStrictToTransitional(f.readXML(drawingXML)))).
		Decode(deWsDr); err != nil && err != io.EOF {
	...
	for _, anchor := range wsDr.TwoCellAnchor {
		if anchor.From != nil && anchor.Pic != nil {
			if drawRel, ok := drawRels[anchor.Pic.BlipFill.Blip.Embed]; ok && drawRel != nil {
				if _, ok := supportedImageTypes[filepath.Ext(drawRel.Target)]; ok {
					ret := filepath.Base(drawRel.Target)
					cellObj, err := CoordinatesToCellName(anchor.From.Col+1, anchor.From.Row+1)
					if err != nil {
...
// 获取所有图片rid和存储位置的map
func (f *File) getDrawingRelationshipsMap(rels string) map[string]*xlsxRelationship {
	if drawingRels, _ := f.relsReader(rels); drawingRels != nil {
		drawingRels.Lock()
		defer drawingRels.Unlock()
	...
	return map[string]*xlsxRelationship{}
}
